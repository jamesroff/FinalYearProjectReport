...
\begin{split}
    ...
    \, , \\
    \dot{p}_t &= \frac{a^2}{V} \left( \dot{m}_{\rm{in}} - \dot{m}(0,t) \right) \, .
\end{split}
\end{equation}

Both of these equations depend upon the mass flow rate though the pipe, $\dot{m}(\eta,t)$. Because the fluid is considered incompressible, this can be calculated as the product of the pipe area $A_p$, fluid density $\rho$, and fluid velocity $v(\eta,t)$. Hence, the mass flow rate can be expressed in terms of the velocity using $\dot{m}(\eta,t) = \rho \, A_p \, v(\eta,t)$.
...
The orthogonality conditions then mean that the only derivative with non-zero coefficient is $\dot{B}_n$. Hence, a separate equation for each temporal derivative of the pressure fluctuation amplitudes can be found. Applying the orthogonality conditions and evaluating the remaining integrals give the generalised equation for $\dot{B}_n(t)$ as
~
\begin{equation*}
\begin{split}
    \dot{B}_n &=
...
As previously mentioned, the derivation for $\dot{C}_n(t)$ is omitted here as involves the same steps applied to the momentum equation. Again, the reader is referred to the appendix for full details of the derivation. After using the Galerkin method on the momentum equation, the generalised equation for $\dot{C}_n(t)$ is
~ 
\begin{equation*}
\begin{split}
    \dot{C}_n &=
...
However, two of the coefficients have a error of $36.3\%$ and $40.0\%$. Both of these coefficients are from the convective terms of the partial differential equation, which are generally negligible for the compressible liquid case~\cite{Hos2016DynamicService}. That the convective effects are negligible will confirmed in \cref{subsec: QWMNonDim} when the dimensionless differential equations are calculated. Hence, the difference between using the Galerkin and Collocation techniques appears largely unimportant.

\section{Quarter-wave model derivation} \label{subsec: QWM Derivation}

As stated previously, the one-mode truncation was found appropriate for the spring-operated case~\cite{Hos2015ModelPipe}. Therefore, one the one-mode truncation will now be considered, corresponding to the acoustic quarter-wave. For a direct spring-operated PRV, using the collocation technique to perform a model reduction was shown to be quantitatively similar to experimental data~\cite{Hos2015DynamicModelling}. Hence, only the coefficients from the collocation technique will be used, which also ensures the following analysis is consistent with the spring-operated case. We begin by ensuring the boundary conditions are satisfied by the quarter-wave model.
%
...
% \end{equation*}
Additionally, both the pressure and velocity distributions, $p(\eta,t)$ and $v(\eta,t)$, are functions of both $B$ and $C$. Therefore, when calculating $\pdiff{p}{t}$ and $\pdiff{v}{t}$ to ensure \cref{eq: FluidPDE} is satisfied, each of two ordinary differential equations contains $\dot{B}$ and $\dot{C}$.
...
%\newpage
The most general simplification is to neglect the fluctuations in velocity, $C(t) \approx 0$, when calculating the boundary condition given by \cref{eq: QWMBoundary1}. As $C(t)$ represents a small amplitude oscillation, the assumption that $v_L(t) + C(t) \approx v_L(t)$ appears valid.
...
The expression for $v_L(t)$ is identical to that for the direct spring-operated case~\cite{Hos2015ModelPipe}, but $p_0(t) \neq p_t(t)$ because the conversion of the tank static pressure into dynamic pressure is considered as discussed in \cref{subsubsec: BoundCond}. Another small fluctuation for which it may be possible to ignore using a similar argument to $C(t)$ (in \cref{eq: QWMBoundary1}) is shown in \textcolor{Red}{red}. For completeness, this \textcolor{Red}{red} term will initially not be neglected, with \textcolor{Red}{red} terms in later equations indicating that those terms occur from considering the small fluctuation, $\textcolor{Red}{B(t)}$ in \cref{eq: QWMPresAndVelBound}.
%
...
\begin{split}
    ...
    \, , \\
    \dot{p}_t &= \frac{a^2}{V} \left( \dot{m}_{\rm{in}} - \rho A_p C(t) - \zeta x(t) \sqrt{\rho} \sqrt{p_t(t) + B(t)} \right) \, .
\end{split}
\end{equation*}

With zero amplitude of the quarter wave, so $B(t) = C(t) = 0$, the equations reduce to the original equations for the valve closing model, \cref{eq: ClosingDiffEqFull}, presented in \cref{sec: Prog}. However, if the ideal acceleration of the fluid for the boundary condition \cref{eq: QWMBoundary1}, then the equation of motion is not identical to previously in \cref{sec: Prog}. The force acting on the main piston due to the change in momentum is counteracted by the decrease in fluid static pressure due to the fluid flow. Neglecting the conversion between dynamic and static conversion would overestimate the force acting on the valve by a factor of two.
%
...
%     \dot{p}_t %&= \frac{a^2}{V} \left( \dot{m}_{\rm{in}} - \dot{m}(0,t) \right)
%     &= \frac{a^2}{V} \left( \dot{m}_{\rm{in}} - \zeta x \sqrt{\rho} \sqrt{p_t(t) + B(t)} - \rho A_p C(t) \right)
% \end{split}
% \end{equation*}
...
\section{Analysis of the quarter-wave model} \label{sec: QWBehaviour}

%We will now display the quarter-wave behaviour of \cref{eq: FullQWMDimensional}.
%The quarter-wave behaviour of \cref{eq: FullQWMDimensional} will now be shown. This is done by fixing the valve closed, so $x = \dot{x} = 0$, and enforcing that the tank pressure remains constant, so $\dot{p}_t = 0$.
Consider first the approximation that the valve is fixed closed, so $x = \dot{x} = 0$, and the tank pressure remains constant, so $\dot{p}_t = 0$.
The  equation for $\dot{C}(t)$ is differentiated again to find an expression for $\ddot{C}(t)$. Applying the closed valve and constant tank pressure conditions gives
~
\begin{equation*}
    \ddot{C} = \frac{\pi \sqrt{2}}{2 L} C \dot{C} - \frac{1}{\rho} \frac{\pi}{2 L} \dot{B} \, .
\end{equation*}
...
~
\begin{equation*}
    \ddot{C} = - \frac{1}{\rho} \frac{\pi}{2 L} \left( \frac{\pi}{2L} a^2 \rho C \right) = - \omega^2 C \, .
\end{equation*}
...
Next, suppose the valve is held open at its equilibrium lift position (even though this is unstable as shown in \cref{sec: Prog}) while again the tank pressure is held constant. Hence, $x = 1$ while $\dot{x} = 0$ and $\dot{p}_t = 0$.
%Instead, the valve will be fixed open at the equilibrium lift. Again, both $\dot{x} = 0$ and $\dot{p}_t = 0$.
Also, any convective and frictional terms are neglected. Differentiating the equation for $\dot{C}(t)$ and applying the fixed valve and constant tank pressure gives
~
\begin{equation*}
...
%     q = \frac{\dot{m}_{\rm{in}}}{\dot{m}_{\rm{cap}}} \, , \quad
...
%     \, , \\ % newline
%     \beta = \frac{a^2}{V} \frac{\dot{m}_{\rm{cap}}}{p_{\rm{ref}} \omega_{\rm{ref}}} \, , \quad
%     \mu = \frac{\rho A_p \omega_{\rm{ref}} x_{\rm{ref}}}{\dot{m}_{\rm{cap}}} \, , \quad
...
Notably, the parameter $\delta$ is only a function of the valve and pipe areas, $A_v$ and $A_p$ respectively. Both of these are physical parameters which are very easy to influence during the design of a valve, so introduces a potentially powerful design parameter.
...
\begin{table}[ht]
    ...
    \begin{tabular}{c|l|r|l}
        ...
        $\dot{m}_{\rm{in}}$ & Mass flow rate into tank & 0 -- 3.8 & \si{kg.s^{-1}} \\ \hline % 0-3.8 Valve 1E2 from \cite{Hos2016DynamicService}
    ...
    \end{tabular}
    \caption{Typical parameter values for a pilot-operated PRV used for the quarter-wave model.}
...
\newpage
\section{Analytical prediction of the pipe-mode instability} \label{subsec: QWMAnalyticalBound}
...
Substituting $\dash{\tilde{C}\,}$ into the equation for $\ddash{\tilde{B}\,}$ yields
~
